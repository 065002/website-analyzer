...
.divider { border: none; border-top: 1px solid rgba(100,116,139,0.12); margin: 1.5rem 0; }
...
.empty-state { text-align: center; padding: 4rem 1rem; animation: fadeUp 0.6s ease both; }
.empty-icon  { font-size: 3.5rem; margin-bottom: 1rem; filter: grayscale(0.3); }
...
def make_radar(radar_data):
    ...
    polar_layout = {k: v for k, v in PLOTLY_LAYOUT.items() if k not in ("xaxis", "yaxis")}
    fig.update_layout(
        **polar_layout,
        polar=dict(
...
def make_treemap_tech(tech_comparison):
    ...
    for domain, cats in tech_comparison.items():
        ...
        parents.append("Tech Stack")
        values.append(0)
        colors_list.append("#1e293b")
    ...
    treemap_layout = {k: v for k, v in PLOTLY_LAYOUT.items() if k not in ("xaxis", "yaxis")}
    fig.update_layout(
        **treemap_layout,
        height=340,
...
<div class="hero-wrap">
    ...
    <div class="hero-sub">
        Technical · SEO · Security · Competitor Analysis<br>
        Instant insights for any website
    </div>
</div>
""", unsafe_allow_html=True)
...
with mode_col:
    ...
    mode = st.radio(
        "",
        ["Single Site", "Competitor Analysis"],
        horizontal=True,
...
if go_btn and mode == "Single Site" and url_input.strip():
    ...
    with g1:
        st.plotly_chart(
            make_gauge(overall, "Overall Score", gauge_color(overall)),
            use_container_width=True,
            config={"displayModeBar": False},
        )
    with g2:
        st.plotly_chart(
            make_gauge(sec_score, "Security Score", gauge_color(sec_score)),
            use_container_width=True,
            config={"displayModeBar": False},
        )
    with g3:
        ...
        st.plotly_chart(
            make_gauge(load_norm, "Speed Score", gauge_color(load_norm)),
            use_container_width=True,
            config={"displayModeBar": False},
        )
    with g4:
        ...
        st.plotly_chart(
            make_gauge(seo_pct, "SEO Completeness", gauge_color(seo_pct)),
            use_container_width=True,
            config={"displayModeBar": False},
        )
    ...
    with t_seo:
        if seo.get("error"):
            ...
            seo_items = {
                ...
                "Image Alt Text":   max(0, 100 - (
                    seo.get("images_missing_alt", 0) / max(seo.get("total_images", 1), 1) * 100
                )),
            }
            bar_colors = [
                "#34d399" if v >= 70 else ("#fbbf24" if v >= 40 else "#f87171")
                for v in seo_items.values()
            ]
            seo_fig = go.Figure(go.Bar(
                x=list(seo_items.values()),
                y=list(seo_items.keys()),
                orientation="h",
            ...
            seo_layout = PLOTLY_LAYOUT.copy()
            seo_layout["xaxis"] = dict(range=[0, 120], gridcolor="rgba(100,116,139,0.08)")
            seo_layout["title"] = dict(
                text="SEO Signal Scores",
                font=dict(size=13, color="#64748b"),
                x=0,
            )
            seo_fig.update_layout(**seo_layout, height=300)
    ...
    with t_sec:
        ...
        with s2:
            ...
            if not sec_headers.get("error"):
                sec_score_val = sec_headers.get("score", 0)
                sec_fig = go.Figure(go.Bar(
                    x=[sec_score_val],
                    y=["Score"],
                    orientation="h",
                    marker=dict(color=gauge_color(sec_score_val), line_width=0),
                    text=[f"{sec_score_val}/100"],
                    textposition="outside",
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    # Radar + bar
    if len(radar_data) >= 2:
        rad_col, bar_col = st.columns(2)
        with rad_col:
            st.plotly_chart(
                make_radar(radar_data),
                use_container_width=True,
                config={"displayModeBar": False},
            )
        with bar_col:
            st.plotly_chart(
                make_bar_comparison(comparison, [
    ...
    for d in domains:
        icon = "🏠 " if d == primary else "🥊 "
        table_html += f"<th>{icon}{d}</th>"
    ...
    for key in show_metrics:
        ...
        row = comparison[key]
        table_html += (
            f'<tr><td style="color:#64748b;font-family:\'DM Mono\',monospace;'
            f'font-size:0.78rem">{row["label"]}</td>'
        )
        ...
        for d in domains:
            ...
            is_worst = (
                ...
                and all(
                    (val <= row["values"].get(od, val))
                    if key not in lower_better
    ...
    for col, site in zip(tech_cols, sites):
        ...
        with col:
            ...
            for cat_label, cat_key, col_kind in [
                ...
                ("Hosting",   "hosting",   "amber"),
            ]:
                items = tc.get(cat_key, [])
                if items:
                    st.markdown(
                        f"<small style='color:#475569'>{cat_label}</small><br>"
                        + tech_badges(items, col_kind),
                        unsafe_allow_html=True,
                    )
    ...
    rec_tabs = st.tabs([
        f"{'🏠' if s['domain'] == primary else '🥊'} {s['domain']}"
        for s in sites
    ])
    for tab, site in zip(rec_tabs, sites):
        with tab:
            ...
            with rc1:
                st.markdown('<p class="sec-head">Critical Issues</p>', unsafe_allow_html=True)
                issues = site_recs.get("critical_issues", [])
                if issues:
                    for issue in issues:
                        st.markdown(f'<div class="issue-row">⚠ {issue}</div>', unsafe_allow_html=True)
                else:
            ...
            with rc2:
                st.markdown('<p class="sec-head">Quick Wins</p>', unsafe_allow_html=True)
                site_wins = site_recs.get("quick_wins", [])
                if site_wins:
                    for win in site_wins:
                        st.markdown(f'<div class="win-row">→ {win}</div>', unsafe_allow_html=True)
                else:
